...
ASSETS_URL=$(curl https://api.github.com/repos/th-ch/youtube-music/releases/latest | jq .assets_url)

LATEST_LINUX_YML_RELASE_URL=$(curl "$ASSETS_URL" | jq '.[0].url')
LATEST_LINUX_YML_DOWNLOAD_URL=$(curl "$LATEST_LINUX_YML_RELASE_URL" | jq '.browser_download_url')
